...
const STATUS_CONFIG = {
  ...
  submitted:          { label:'Enviado',            color:'#d97706', bg:'#fff7ed', icon:Send          },
  reviewed:           { label:'Revisado ✓',         color:'#2e5244', bg:'#f0f7f4', icon:CheckCircle2  },
...
// ─────────────────────────────────────────────────────────────────────────────
// Modal de vista previa del contenido del informe
// ─────────────────────────────────────────────────────────────────────────────
    ...
    <Dialog open onOpenChange={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3 shrink-0"
          ...
          <div>
            <p className="text-white font-semibold text-sm">{report.consecutive} · {report.title}</p>
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-md" aria-describedby="review-desc">
        ...
        <div id="review-desc" className="space-y-4 py-1">
          <div className="rounded-xl p-4 space-y-1.5 text-sm" style={{ background:'#f5f5ef' }}>
            {[
              ['Consecutivo',   report.consecutive],
              ['Título',        report.title],
          ...
          <div>
            <Label className="text-sm font-medium text-gray-700 mb-1.5 block">
              ...
              {approved  && <span className="text-gray-400 font-normal"> (opcional)</span>}
            </Label>
            <Textarea value={notes} onChange={e=>setNotes(e.target.value)}
              placeholder={approved ? 'Comentarios adicionales...' : 'Indica qué debe corregir el usuario...'}
              rows={3} className="resize-none text-sm"/>
            <p className="text-xs text-gray-400 mt-1">Se enviará notificación al autor del informe.</p>
          </div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose} disabled={saving}>Cancelar</Button>
          <Button
            onClick={() => { if (!approved && !notes.trim()) return; onConfirm({ approved, notes }); }}
            disabled={saving || (!approved && !notes.trim())}
            style={{ background: approved?'#2e5244':'#dc2626', color:'white' }}>
            {saving ? 'Guardando...' : (approved ? 'Aprobar informe' : 'Solicitar corrección')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
export default function InformesManager({ onBack }) {
  ...
  // ← Destructurar TODOS los valores que se usan, incluyendo loadFullReportData
  const {
    ...
    deleteInforme,
    getSignedUrl,
    loadFullReportData,      // ← crítico: debe estar en el return del hook
  } = useInformes();
  ...
  const filtered = informes.filter(r => {
    ...
    const matchSearch = !search || [r.title, r.consecutive, r.period, r.process?.name, r.creator?.full_name]
  ...
  const handleReview = async (decision) => {
    ...
    setSaving(false);
    if (result.success) { setReviewTarget(null); showToast(decision.approved ? 'Informe aprobado' : 'Corrección solicitada'); }
    else showToast(result.error??'Error al revisar','error');
  };
  ...
  const handleDelete = async () => {
    ...
    result.success ? showToast('Informe eliminado') : showToast(result.error??'Error al eliminar','error');
  };

  const openPdf = async (report) => {
    if (!report.pdf_url) return;
    try { const url = await getSignedUrl(report.pdf_url); window.open(url,'_blank'); }
    catch { showToast('No se pudo abrir el PDF','error'); }
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatTile label="Enviados"          value={stats.submitted} icon={Send}          color="#d97706" bg="#fff7ed"/>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-gray-400 text-sm font-medium">
            {informes.length===0 ? 'Aún no hay informes en el sistema' : 'Sin resultados para los filtros aplicados'}
          </p>
                  ...
                  return (
                    <motion.tr key={r.id}
                      ...
                      <td className="px-3 py-2.5">
                        <span className="font-mono text-xs font-bold text-[#2e5244]">{r.consecutive}</span>
                      </td>
                      ...
                      <td className="px-3 py-2.5">
                        <div className="flex items-center gap-0.5">
                          ...
                          <button title="Visualizar contenido"
      ...
      {/* Modal vista previa — pasa loadFullReportData como onLoadData */}
      {previewTarget && (
        <PreviewModal
      ...
      {/* Modal revisión */}
      {reviewTarget && (
        <ReviewModal report={reviewTarget} onConfirm={handleReview}
          onClose={() => setReviewTarget(null)} saving={saving}/>
      )}
      ...
      {deleteTarget && (
        <Dialog open onOpenChange={() => setDeleteTarget(null)}>
          <DialogContent className="max-w-sm" aria-describedby="delete-desc">
            ...
            <p id="delete-desc" className="text-sm text-gray-600 py-2">
              ¿Eliminar <strong>{deleteTarget.title}</strong> ({deleteTarget.consecutive})?
